...
(async () => {
  ...
  ok("service worker cache bumped to tsb-v235", swSrc.includes('tsb-v235'));
  ok("key files ship ?v=235", idxH.includes("css/style.css?v=235") && lgSrc2.includes("v=235"));
  ...
  ok("Settings page has an Install-to-home-screen option", setH.includes('data-install') && setH.includes("Install this app to your home screen") && setH.includes("js/install.js?v=235"));
  ok("You window has the Install row too", youH.includes('data-install') && youH.includes("js/install.js?v=235"));
  ...
  ok("Build markers say tsb-v235 (settings + You window)", setH.includes("Build tsb-v235") && youH.includes("Build tsb-v235"));
  let staleBuilds = [];
  for (const f of ["settings.html","login.html","index.html","about.html","scan.html","book.html","graveyard.html"]) {
    ...
    const wantVer = (swSrc.match(/CACHE_VERSION = "(tsb-v\d+)"/) || [])[1] || "tsb-v235";
  ...
  ok("v222: Settings has the one-tap upload test", setH3.includes('id="upDiag"') && setH3.includes("Test upload right now") && setH3.includes("Build tsb-v235"));
  ...
  ok("v225: You-window banner shows real logo stack + loads store.css", loginV4.includes("st-banner__logos") && loginV4.includes("assets/logos/amazon.svg") && loginV4.includes("css/store.css?v=235"));
