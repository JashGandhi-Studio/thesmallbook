...
(async () => {
  ...
  ok("service worker cache bumped to tsb-v245", swSrc.includes('tsb-v245'));
  ok("key files ship ?v=245", idxH.includes("css/style.css?v=245") && lgSrc2.includes("v=245"));
  ...
  ok("Settings page has an Install-to-home-screen option", setH.includes('data-install') && setH.includes("Install this app to your home screen") && setH.includes("js/install.js?v=245"));
  ok("You window has the Install row too", youH.includes('data-install') && youH.includes("js/install.js?v=245"));
  ...
  ok("Build markers say tsb-v245 (settings + You window)", setH.includes("Build tsb-v245") && youH.includes("Build tsb-v245"));
  let staleBuilds = [];
  for (const f of ["settings.html","login.html","index.html","about.html","scan.html","book.html","graveyard.html"]) {
    ...
    const wantVer = (swSrc.match(/CACHE_VERSION = "(tsb-v\d+)"/) || [])[1] || "tsb-v245";
  ...
  ok("v222: Settings has the one-tap upload test", setH3.includes('id="upDiag"') && setH3.includes("Test upload right now") && setH3.includes("Build tsb-v245"));
  ...
  ok("v225: You-window banner shows real logo stack + loads store.css", loginV4.includes("st-banner__logos") && loginV4.includes("assets/logos/amazon.svg") && loginV4.includes("css/store.css?v=245"));
