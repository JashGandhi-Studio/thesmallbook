...
(function () {
  ...
  const backup = {
    export() {
      const data = {};
      /* v268 · the identity wall: a backup carries THIS account's data.
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => {
      ...
      /* v274 FIX: a worker handover must NEVER reload the page. The old
         handover-reload raced the new worker's activation and the
         old cache deletion, wedging phones in an endless loading state.
         Freshness is bootguard.js's job now (one purge per build, never
         overlapping a navigation). */
      void 0;
    });
  }
})();
...
/* v274: the page booted, tell the watchdog it can stand down */
