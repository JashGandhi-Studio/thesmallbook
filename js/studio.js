...
(function () {
  ...
  function loadLocal(src, useCors) {
    return new Promise(function (res) {
      var im = new Image();
      if (useCors) { try { im.crossOrigin = "anonymous"; } catch (e) {} }
  ...
  async function compose(o) {
    ...
    try {
      ...
      if (src) {
        ...
        /* v268 · try clean, then CORS-flagged; a raw cross-origin load is the
           last resort precisely BECAUSE it taints the canvas, if even that
           fails, the card gets composed on our own background instead. */
        local = await loadLocal(u, /^https?:/.test(u) && !/^blob:/.test(u));
        if (!local && u !== src) local = await loadLocal(src, true);
        if (local) {
          /* phones hand us 12–48 MP photos; some mobile canvases die quietly
             past ~16 MP, draw from a sane copy so compose never blank-fails */
          try {
            var MW = 2200, MH = 2200;
            if (local.naturalWidth > MW || local.naturalHeight > MH) {
              var sc = Math.min(MW / local.naturalWidth, MH / local.naturalHeight);
              var t = document.createElement("canvas");
              t.width = Math.max(1, Math.round(local.naturalWidth * sc));
              t.height = Math.max(1, Math.round(local.naturalHeight * sc));
              t.getContext("2d").drawImage(local, 0, 0, t.width, t.height);
              local = await loadLocal(t.toDataURL("image/jpeg", 0.92), false);
            }
          } catch (e) {}
        }
      }
      ...
      var canvas;
      try {
        canvas = await render(false, { mark: (o.mark === true) });
        return await canvasToFile(canvas, "thesmallbook-card.png");
      } catch (drawErr) {
        /* the photo tainted or broke the canvas, the QUOTE still deserves its
           card: same words, our background, zero silence about it */
        console.warn("TSB_STUDIO compose: photo failed (" + (drawErr && drawErr.message) + "), composing on the house background");
        img = null;
        if (!o.bg) S.bg = "img-pastel";
        canvas = await render(false, { mark: (o.mark === true) });
        compose.lastFallback = "photo";
        return await canvasToFile(canvas, "thesmallbook-card.png");
      }
    } finally {
